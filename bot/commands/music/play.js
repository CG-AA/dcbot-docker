...
module.exports = {
  ...
  async execute({ inter, client }) {
	...
	res.playlist ? queue.addTrack(res.tracks) : queue.addTrack(res.tracks[0]);
	  ...
	  if (res.playlist) {
		await inter.followUp(`Added ${res.tracks.length} tracks from ${res.playlist.title}`);
	  } else {
		await inter.followUp(`Added ${
			queue.tracks.toArray()[0]
			  ? queue.tracks.toArray()[queue.size - 1]
			  : res.tracks[0]
		  }`);
	  }
