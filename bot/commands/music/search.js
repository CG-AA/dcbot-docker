...
module.exports = {
    ...
    async execute({ client, inter }) {
        ...
        const maxTracks = res.tracks.slice(0, client.config.opt.searchMaxResult);
        ...
        .setTimestamp()
        .setFooter({ text: 'Provided From MusicOne', iconURL: inter.member.avatarURL({ dynamic: true })})

        inter.editReply({ embeds: [embed] });

        const collector = inter.channel.createMessageCollector({
            time: client.config.opt.searchReactionTimeLimit,
            max: 1,
            errors: ['time'],
        ...
        collector.on('collect', async (query) => {
			...
			await inter.followUp(`Added ${res.tracks[query.content - 1]}`);
